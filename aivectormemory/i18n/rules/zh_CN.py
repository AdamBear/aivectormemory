...
DEV_WORKFLOW_PROMPT = (
    ...
    "---\n\n"
    "## ⚠️ 自测要求\n\n"
    "**禁止让用户手动操作** - 能自己执行的，不要让用户做，自测通过后才能说\"等待验证\"。\n\n"
    "- **纯后端/非前端变更**：用 pytest、API 请求或脚本验证功能正确性\n"
    "- **MCP Server**：通过 stdio 发送 JSON-RPC 消息验证\n"
    "- **涉及前端可见数据变更**（数据库修改、API 返回值变更、前端代码修改）：**必须使用 Playwright 验证前端页面展示结果**，禁止仅用 SQL 查询、curl、python 脚本验证就声称\"已通过\"。服务未启动时必须先启动服务再验证，禁止以\"服务未启动\"为由跳过 Playwright\n"
    "- 自测通过后才能说\"等待验证\"\n\n"
    "---\n\n"
    "## ⚠️ 开发规则\n\n"
    "> 开发完成必须进行自测。\n"
